...
function onSaveBtn()
{
  ...
  if (title != '')
  {
    addMemo(title,sound,content);
  }
}

function addMemo(title,sound,content) {
  var memo = MC.Collection("Memo");

  memo.insert({ title: title, sound: sound, content: content})
...
function onShowLink(id,title,sound,content)
{
  currentMemoID = id;
...
function onEditBtn()
{
  ...
  var sound = $("#sound_show").text();
  var content = $("#content_show").text();
  $("#title_edit").val(title);
  $("#sound_edit").text(sound);
...
function onUpdateBtn()
{
  ...
  var new_sound = $("#sound_edit").val();
  var new_content = $("#content_edit").val();
  var id = currentMemoID;
  if (new_title != '') {
    editMemo(id, new_title, new_sound ,new_content);
  }
}

function editMemo(id, new_title, new_sound ,new_content){
  var memo = MC.Collection("Memo");
  memo.findMine(MC.Criteria("_id==?", [id]))
    .done(function(items, totalItems)
    {
      items.items[0].title = new_title;
      items.items[0].sound = new_sound;
